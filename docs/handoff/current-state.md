...
- **Code-driven documentation, M1–M3.5 (plan 2026-04-29)**: per-source provenance (`apis.source`, V13); OpenAPI ingestion produces source-tagged `apis` rows; M2 adds per-endpoint Confluence pages (V14 adds `apis.confluence_page_id`) with service-page hyperlinks; M2.5 added soft-delete cleanup (since superseded). M3 adds `services.module_path`, `services.tests_page_id`, and `service_test_scenarios` (V16) plus the four-piece pipeline (RepoFileFetcher → JavaTestExtractor → CodeSyncCoordinator.refreshTests → TestScenariosPageRenderer + SyncCoordinator.syncTestsPage). **M3.5 retrofitted ingestion to append-only**: V17 adds `observed_at` + `presence` to `apis` and `service_test_scenarios`; V18 (vendor-split between `db/migration_postgresql/` and `db/migration_mariadb/`) drops the unique constraints. Code-sync writes only inserts — disappearance recorded as `presence='absent'` tombstones, content changes recorded as new observations carrying forward the previous `confluence_page_id`. Live readers use window-function "latest per key where presence='present'" queries. `confluence_page_id` is the only mutable bookkeeping column; the cleanup pass nulls it on tombstones after deleting the Confluence page. atlas-intake dogfoods itself end-to-end: exposes `/v3/api-docs` via Springdoc with `@Operation` summaries; `POST /api/code-sync/refresh/{serviceId}` round-trips against Postgres; `POST /api/code-sync/refresh-tests/{serviceId}` round-trips against GitHub.
...
`docs/plans/2026-04-29-code-driven-documentation.md` — five-milestone shift from intake-only to a hybrid where APIs, tests, and pom-derived metadata are auto-refreshed from code. M1, M2, M2.5, M3, and M3.5 (append-only retrofit per the project rule "persist as observed; updates decorate, never overwrite") closed. Decision recorded at M3.8: code-sync stays folded into `atlas-intake` for the prototype; production team has a clean seam (the `com.atlas.codesync` package) to lift later. Next: M4 (pom.xml ingestion, designed against the new append-only model from the start). M5 closes with phase-level reflection on whether to push into AI-narrated walkthroughs or production-readiness next.

Carry-overs to track: stale `/api/smoke/anthropic` intake row in dogfood DB (M5 cleanup); private-repo GitHub auth (new DD-014 candidate; current fetcher is public-only); test-name normalisation for nicer "Tests" page rendering (polish follow-up); sync-side live-verify of M2/M2.5/M3/M3.5 against the real ATLAS space deferred to user discretion; MariaDB project rule about VARCHAR(191) for unique-index columns worth promoting to CLAUDE.md Constraints; live-verify of the append-only retrofit against the user's local DB (V17 + V18 will apply on next atlas-intake boot); production-scale window-function index would benefit from `apis.method`/`apis.path` becoming bounded VARCHAR.
