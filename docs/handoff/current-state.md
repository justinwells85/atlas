...
- **Code-driven documentation, M1 + M2 + M2.5 (plan 2026-04-29)**: per-source provenance landed (`apis.source` + V13 migration); OpenAPI ingestion produces openapi-source `apis` rows from a remote spec, with intake-owned endpoints skipped to honour the unique constraint and `@Transactional` rollback on partial failure. M2 adds per-endpoint Confluence pages parented under the service page (V14 adds `apis.confluence_page_id`), and the service-page APIs section linkifies to them. M2.5 adds `apis.deleted_at` (V15) so when code-sync drops a row the cleanup pass deletes its endpoint page on the next sync — same shape as ADR-014 for services. atlas-intake dogfoods itself: exposes `/v3/api-docs` via Springdoc with `@Operation` summaries, and `POST /api/code-sync/refresh/{serviceId}` round-trips a refresh against Postgres.
- **186 active tests, 0 failures** across all four modules (atlas-domain 33, atlas-intake 50, atlas-mcp 24, atlas-confluence-sync 79). Behavior-focused per ADR-006; mocks only at architectural seams (Anthropic, Confluence, remote OpenAPI hosts).
...
| atlas-confluence-sync | 79 | Renderer output for full/partial/minimal services; ConfluenceClient HTTP shape via WireMock; SyncCoordinator orchestration including 404-recreate, service-level orphan-cleanup, per-endpoint page lifecycle (M2), and endpoint-page orphan cleanup (M2.5); architecture-map renderer; ApiEndpointPageRenderer; SyncController endpoints. |
...
`docs/plans/2026-04-29-code-driven-documentation.md` — five-milestone shift from intake-only to a hybrid where APIs, tests, and pom-derived metadata are auto-refreshed from code. M1 (provenance + OpenAPI ingestion), M2 (per-endpoint Confluence pages), and M2.5 (endpoint-page orphan cleanup) closed. Next: M3 (test-method extraction → "What this service guarantees" page). M5 closes with phase-level reflection on whether to push into AI-narrated walkthroughs or production-readiness next.

Carry-overs to track: stale intake row at `/api/smoke/anthropic` in the dogfood DB (M5 cleanup); code-sync still folded into `atlas-intake` (decision point at end of M3); sync-side live-verify of M2/M2.5 against the real ATLAS space deferred to user discretion; reactivate-on-collision for re-adding a recently-removed endpoint within the same syncAll cycle (production-readiness item; not blocking).
